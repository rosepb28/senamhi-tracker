...
class ForecastScheduler:
    ...
    def start(self):
        ...
        logger.info("=" * 60)
        logger.info(f"Forecast interval: Every {settings.forecast_scrape_interval} hours")
        logger.info(f"Warnings interval: Every {settings.warning_scrape_interval} hours")
        ...
        schedule.every(settings.forecast_scrape_interval).hours.do(run_forecast_scrape_job)
        
        # Schedule warnings job
        schedule.every(settings.warning_scrape_interval).hours.do(run_warnings_scrape_job)
        ...
        if jobs:
            logger.info("\nScheduled jobs:")
            for job in jobs:
                logger.info(f"  - {job.job_func.__name__}: next run at {job.next_run.strftime('%Y-%m-%d %H:%M:%S')}")
