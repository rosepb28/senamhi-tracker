...
def get_warning_all_details(warning_number):
    ...
    try:
        details = (
            db.query(WarningDailyDetail)
            .filter_by(warning_number=warning_number)
            .order_by(WarningDailyDetail.department, WarningDailyDetail.day_number)
            .all()
        )

        if not details:
            return jsonify({"error": "No details found for this warning"}), 404
        ...
        return jsonify(
            {"warning_number": warning_number, "departments": list(grouped.values())}
        )
...
def get_warning_details_by_department(warning_number, department):
    ...
    try:
        details = (
            db.query(WarningDailyDetail)
            .filter_by(warning_number=warning_number, department=department.upper())
            .order_by(WarningDailyDetail.day_number)
            .all()
        )
        ...
        warning = (
            db.query(WarningAlert).filter_by(warning_number=warning_number).first()
        )
                ...
                "days": [
                    {
                        ...
                        "date": (base_date + timedelta(days=d.day_number - 1)).strftime(
                            "%Y-%m-%d"
                        )
                        if base_date
                        else None,
